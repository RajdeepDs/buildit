...
You can start up for example only the web application by using `bun turbo dev --filter=@buildit/web`.
...
You can build for example only the web application by using `bun turbo build --filter=@buildit/web`.
...
You can invoke these commands using `bun`, for example to check for collisions and inconsistencies between your database and the schema, you can run `bun --filter @buildit/db db:check`.
