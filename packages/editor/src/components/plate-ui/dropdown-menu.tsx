...
const menuItemVariants = cva(
  cn(
    'relative flex cursor-pointer select-none items-center rounded-sm px-2 py-1.5 text-sm outline-none transition-colors bg-white text-surface',
    'hover:bg-weak hover:text-strong focus:bg-soft focus:text-strong data-[disabled]:pointer-events-none data-[disabled]:opacity-50 dark:focus:bg-slate-800 dark:focus:text-slate-50',
  ),
  {
    variants: {
      inset: {
...
>(({ children, className, hideIcon, ...props }, ref) => (
  <DropdownMenuPrimitive.RadioItem
    className={cn(
      'flex items-center justify-between font-normal text-sm text-strong hover:bg-weak cursor-pointer rounded outline-none',
      'px-2 py-1',
      className,
    )}
    ref={ref}
  ...
  >
    ...
    {!hideIcon && (
      <div className='flex size-3.5 items-center justify-center'>
        <DropdownMenuPrimitive.ItemIndicator>
          <Icons.check className='size-4 text-sub' />
        </DropdownMenuPrimitive.ItemIndicator>
      </div>
    )}
  </DropdownMenuPrimitive.RadioItem>
